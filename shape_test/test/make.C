void make(int xxx=0){
  ...
  TFile *origin = new TFile("Bkg_VS_signal_s_ch_id.root");
  ...
  for(unsigned int i=0; i<masses.size(); i++){
    ...
    TH1D *signal_den = (TH1D *)origin->Get(channel+"/Den_"+mass+"_h_OS_lljjjjmass_SR1_DiMu");
    ...
    for(int i=1; i<=background->GetXaxis()->GetNbins(); i++){
      for(int j=i+1; j<=background->GetXaxis()->GetNbins(); j++){
        double this_bkg = background->Integral(i,j);
        double this_eff = signal->Integral(i,j);
        //cout << i << "\t" << background->GetXaxis()->GetNbins() << " : " << this_bkg << "\t" << this_eff << endl;
        if(this_eff/(1.+sqrt(this_bkg)) > opt_punzi){
          opt_obs = data_obs->Integral(i,j);
          opt_bkgd = this_bkg;
          opt_sig = this_eff;
          opt_punzi = this_eff/(1.+sqrt(this_bkg));
          opt_cut_left = background->GetXaxis()->GetBinLowEdge(i);
